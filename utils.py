...
def write_on_file(filename, open_mode, str):
...
def autoencoder_cross_validation(features_extractor, batch_size, X_train, X_valid, features_mean, features_std,
    ...
    if os.path.isfile(os.path.join('./', 'autoencoders', dataset + '_autoencoder.h5')):
        best_model = keras.models.load_model(os.path.join('./', 'autoencoders', dataset + '_autoencoder.h5'))
        write_on_file(result_filename, 'a',
                      "[%s] Best autoencoder loaded from an existing .h5 file (cross validation has been skipped)." % dataset.upper())
    else:
        best_model = None
        best_loss = None
...
def get_loss(loss):
    if loss == 'mean_squared_error':
        ...
        raise ValueError('specified loss is not available!')
...
def expert_cross_validation(model_name, batch_size, X_train, y_train, X_valid, y_valid, n_classes,
    ...
    if os.path.isfile(os.path.join('./', 'experts', dataset + '_expert.h5')):
        best_model = keras.models.load_model(os.path.join('./', 'experts', dataset + '_expert.h5'))
        write_on_file(result_filename, 'a',
                ...
                for epsilon in epsilons:
                    ...
                    if data_augmentation:
                        ...
                        expert.fit_generator(
                            ...
                            callbacks=[EarlyStopping(monitor='val_acc', mode='max', verbose=1, patience=5)])
                    else:
                        expert.fit(X_train, to_categorical(y_train), batch_size=batch_size, epochs=epochs, verboscae=2,
                                   callbacks=[EarlyStopping(monitor='val_acc', mode='max', verbose=1, patience=5)])
                    expert.fit(X_train, to_categorical(y_train), batch_size=batch_size, epochs=epochs, verbose=2)
...
def standardize_features(features, mean, std):
    input = layers.Input(shape=(features.shape[1],))
    output = layers.Activation(activation='sigmoid')(input)
    model = keras.Model(inputs=input, outputs=output)
...
def autoencoder_cross_validation(features_extractor, batch_size, X_train, X_valid, features_mean, features_std,
    ...
    best_model = None
    best_loss = None
    best_params = {'hidden_layer_size': None, 'weight_decay': None, 'learning_rate': None, 'epsilon': None}

    train_features = standardize_features(features_extractor.predict(X_train), features_mean, features_std)
    valid_features = standardize_features(features_extractor.predict(X_valid), features_mean, features_std)
...
def expert_cross_validation(model_name, batch_size, X_train, y_train, X_valid, y_valid, n_classes,
                            weight_decays, learning_rates, epsilons, epochs, dataset, data_augmentation):
    data_size = X_train[0].shape[0]
    experts_dir = '/home/vmieuli/experts'
    expert_filename = dataset + '_' + str(data_size) + '_' + model_name + '_expert.h5'

    if os.path.isfile(os.path.join(experts_dir, expert_filename)):
        best_model = load_model(os.path.join(experts_dir, expert_filename))
        write_on_file(result_filename, 'a',
                      "[%s] Best expert loaded from an existing .h5 file (cross validation has been skipped)." % dataset.upper())
        print(
            "[%s] Best expert loaded from an existing .h5 file (cross validation has been skipped)." % dataset.upper())
    else:
        best_model, best_acc, best_loss = None, None, None
        best_params = {'weight_decay': None, 'learning_rate': None, 'epsilon': None}
        ...
        for weight_decay in weight_decays:
            for learning_rate in learning_rates:
                for epsilon in epsilons:
                    ...
                    if data_augmentation:
                        image_generator = ImageDataGenerator(rotation_range=15, width_shift_range=.15,
                    ...
                    valid_loss, valid_acc = expert.evaluate(X_valid, to_categorical(y_valid), verbose=0)
                    if best_acc is None or ((best_acc is not None) and valid_acc > best_acc):
                        best_model, best_acc, best_loss = expert, valid_acc, valid_loss
                        best_params['weight_decay'] = weight_decay
                        best_params['learning_rate'] = learning_rate
                        best_params['epsilon'] = epsilon
                    write_on_file(result_filename, 'a',
                                  "[%s] valid_acc=%f - valid_loss=%f | weight_decay=%f - learning_rate=%f - epsilon=%.8f | ETA=%f"
                                  % (dataset.upper(), valid_acc, valid_loss, weight_decay, learning_rate, epsilon,
                                     time.time() - start_time))
                    print(
                        "[%s] valid_acc=%f - valid_loss=%f | weight_decay=%f - learning_rate=%f - epsilon=%.8f | ETA=%f"
                        % (dataset.upper(), valid_acc, valid_loss, weight_decay, learning_rate, epsilon,
...
def compute_expert_accuracy(expert, X, y, guest_indexes):
    correct_predictions = 0
    y_filtered = y[guest_indexes]
    predicted_probabilities = expert.predict(X[guest_indexes], verbose=0)
    for i in range(len(predicted_probabilities)):
        if np.argmax(predicted_probabilities[i]) == y_filtered[i]:
            correct_predictions += 1
    return correct_predictions / len(X)
